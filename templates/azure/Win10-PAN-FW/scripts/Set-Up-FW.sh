...
until curl --silent -k -X GET curl -k -X GET "https://$PRIVATE_IP/api/?type=keygen&user=$ADMIN_USER&password=$ADMIN_PASSWORD" --output /dev/null; do
    echo "$INFO_TAG Waiting for PAN FW to be up.."
    sleep 5
done

# Get API Key
API_RESPONSE=$(curl --silent -k -X GET curl -k -X GET "https://$PRIVATE_IP/api/?type=keygen&user=$ADMIN_USER&password=$ADMIN_PASSWORD" --output /dev/null)
...
sed -i "s/DEMO-USER/${ADMIN_USER}/g" azure-sample.xml
sed -i "s/DEMO-PASSWORD-HASH/${PW_HASH}/g" azure-sample.xml

# Set up PAN FW
python Config-FW.py $API_KEY $PRIVATE_IP